import{c as e,l as t}from"./main-DS7Wfilc.js";var n=t(e(),1);async function r(){try{let e=await fetch(`/api/menu`);if(!e.ok){a();return}let t=await e.json(),n=document.getElementById(`dynamic-category-nav`);n&&(n.innerHTML=t.categories.sort((e,t)=>e.order-t.order).map(e=>`
          ...
          `).join(``));let r={soups:`Broths & slow-simmered warmth`,appetizers:`Bold starters & sharing plates`,tandoor:`Clay oven crafted`,"tandoor-treasures":`Clay oven crafted`,mains:`Curries & slow-cooked gravies`,biryani:`Muglai saffron standard`,mandi:`Arabic smoked rice`,beverages:`Teas & cold mocktails`,chinese:`Wok tossed`,momo:`Steamed or fried`,breads:`Tandoor baked`,desserts:`Sweet endings`},o=document.getElementById(`dynamic-menu-sections`);if(o){let e=``;t.categories.sort((e,t)=>e.order-t.order).forEach(n=>{let a=t.products.filter(e=>e.category===n.name);if(a.length===0)return;let o=r[n.id]||`Freshly prepared`,s=a.filter(e=>e.diet===`VEG`),c=a.filter(e=>e.diet!==`VEG`);e+=`
